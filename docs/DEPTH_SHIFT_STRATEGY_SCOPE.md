...
- Handling several of these markets *concurrently and continuously* —
  ...
  always-on collector.
- **Resolved — see §6 below for the answer.** Two independent sessions
  researched this same open question three days apart without either
  knowing about the other (a stale local git clone on the second session's
  side, caught only at merge time — noted here for the record rather than
  quietly smoothed over). §6 (2026-09-10) is the stronger, canonical
  answer: it actually opened a live connection and captured real messages,
  not just read documentation. A second, doc-only-confirmed pass reached
  the same yes/no conclusion (WS channel exists, same endpoint, same
  subscribe shape) on 2026-09-13 but added nothing §6 doesn't already
  cover in more depth — superseded, kept out of here to avoid two
  competing "the answer is" blocks in the same section.
